...
class RedactionUnavailable(RuntimeError):
    """Raised when the Presidio analyzer can't be reached (model missing,
    NLP backend down, etc.).  The proxy catches this and returns 503 to
    upstream so the request is NOT forwarded with raw PII intact.

    Defined here so `from .redactor import RedactionUnavailable` in
    main.py resolves on the local copy (the VM version of redactor.py
    had this already; this file lagged behind).
    """
    pass
