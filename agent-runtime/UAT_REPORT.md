...
| F15 | **High** | `app/proposals/assignees.py` | Proactive `recurring_ritual` / `reflective_heuristic` / `contradiction_arc` / `stance_arc` proposals routed to a **critic** as the task OWNER — but critics have no `write_file`/`attach_output`/`submit_for_review` (§4.4) → task dead-ends (IC composes inline, raises blocker, NON-CONVERGENCE). Caught live in the sidebar. | **Fixed** — proposal owners are always producers (researcher); critic split applies at review time only; guard coerces any critic mapping → researcher (+5 tests); validated live to `done` |
